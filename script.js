...
function resetPegs(n) {
  ...
  [pegA, pegB, pegC].forEach(p => (p.innerHTML = '<div class="peg-label">' + p.id.slice(-1) + '</div>'));

  for (let i = n; i >= 1; i--) {
    const disk = document.createElement('div');
    disk.classList.add('disk');
    disk.style.width = 40 + i * 18 + 'px';
    disk.style.bottom = (n - i) * 26 + 'px';
    disk.style.backgroundColor = `hsl(${i * 45}, 80%, 55%)`;
    disk.textContent = i;
...
function animateMoves() {
  ...
  disk.style.bottom = (pegs[to].length - 1) * 26 + 'px';
